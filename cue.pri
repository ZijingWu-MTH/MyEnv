...
ucd = python3 env{myEnvFolder}/generate_goto_dir_script.py $1 up  env{TMP}/goto.{shell_ext} -e "cd {folder}" -f -d {command_sep} . env{TMP}/goto.{shell_ext}
dcd = python3 env{myEnvFolder}/generate_goto_dir_script.py $1 down  env{TMP}/goto.{shell_ext} -e "cd {folder}" -f -d {command_sep} . env{TMP}/goto.{shell_ext}
dvim = python3 env{myEnvFolder}/generate_goto_dir_script.py $1 down env{TMP}/vim.{shell_ext} -e "vim {path}" -f {command_sep} . env{TMP}/vim.{shell_ext}
