...
sys.path.append(os.path.join(os.path.dirname(__file__), 'tools/telnetsrv/'))
...
# a separate object to deal with any logic that needs to persist
...
'''
    ...
    @command('openurl')
    def cmdOpenUrl(self, params):
        url = ' '.join(params)
        subprocess.call(["/usr/bin/open", url])
        self.writeresponse("open url finished")
