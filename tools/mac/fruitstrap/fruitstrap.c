...
CFStringRef copy_developer_disk_image_path(AMDeviceRef device) {
    ...
    if (!found) {
        path = CFStringCreateWithFormat(NULL, NULL, CFSTR("/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/DeviceSupport/5.1/DeveloperDiskImage.dmg"));
        printf("Find the dmg at %s for version %s\n", CFStringGetCStringPtr(path, kCFStringEncodingMacRoman), CFStringGetCStringPtr(version, kCFStringEncodingMacRoman));
        found = path_exists(path);
    }
