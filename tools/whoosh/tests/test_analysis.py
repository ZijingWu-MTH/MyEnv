...
def test_path():
    ...
    PATH = fields.FieldType(formats.Existence(), analysis.PathTokenizer())
    schema = fields.Schema(desc=fields.KEYWORD, category=PATH, location=PATH)
    ix = RamStorage().create_index(schema)

    data = [
    ["new blue", "auto/bike/touring", "england/brighton"],
    ["used red", "auto/car/coupe", "england/london/sw"],
    ["used red", "auto/van/cube", "canada/ontario/toronto"],
    ["new red", "home/furn/dining", "usa/ca"],
    ["new blue", "home/furn/patio", "jamaica/kingston"],
    ["used red", "auto/car/wagon", "usa/ca/orange"],
    ]

    with ix.writer() as w:
        for desc, category, location in data:
            w.add_document(desc=u(desc), category=u(category),
                           location=u(location))

    with ix.searcher() as s:
        qp = qparser.QueryParser("desc", ix.schema)

        roots = [text for text in s.lexicon("category")
                 if "/" not in text]

        q = qp.parse(u("red"))
        cat = sorting.FieldFacet("category", allow_overlap=True)
        loc = sorting.FieldFacet("location", allow_overlap=True)
        r = s.search(q, groupedby=[cat, loc])
        assert_equal(set(r.groups("category").keys()),
                     set(["auto", "auto/car", "auto/car/coupe",
                          "auto/van", "auto/van/cube", "home", "home/furn",
                          "home/furn/dining", "auto/car/wagon"]))

        assert_equal(set(r.groups("location").keys()),
                     set(["england", "england/london", "england/london/sw",
                          "canada", "canada/ontario", "canada/ontario/toronto",
                          "usa", "usa/ca", "usa/ca/orange"]))
